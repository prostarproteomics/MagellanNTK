...
mod_nav_pipeline_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    #   tagList(
    #     lapply(seq_len(length(rv$config$ll.UI)), function(i) {
    #       if (i==1)
    ...
    GetMaxValidated_AllSteps = function(){
      ...
      val <- 0
      ind <- grep(global$VALIDATED, rv$steps.status)
      if (length(ind) > 0) 
        val <-max(ind)
      val
    }
    ...
    Set_All_Skipped = function(){
      ...
      rv$steps.status <- setNames(rep(global$SKIPPED, rv$length), 
                                          rv$config$steps)
    }
    ...
    Discover_Skipped_Steps = function(){
      ...
      for (i in seq_len(rv$length)){
        ...
        if (rv$steps.status[i] != global$VALIDATED && max.val > i)
          rv$steps.status[i] <- global$SKIPPED
      }
    }
    ...
    observeEvent(input$modal_ok, ignoreInit=FALSE, ignoreNULL = TRUE, {
      ...
      #browser()
      #rv$steps.reset <- input$rstBtn + reset()
      #Set_All_Reset()
      LocalReset()
    ...
    output$SkippedInfoPanel <- renderUI({
      ...
      #entire_process_skipped <- isTRUE(sum(rv$steps.status) == global$SKIPPED * rv$length)
    ...
    # observeEvent(req(rv$proc$status()), {
    #   #browser()
    #   # If step 1 has been validated, then initialize rv$dataIn
    #   if (rv$steps.status[1]==0 && rv$proc$status()[1]==1)
    #     rv$dataIn <- rv$temp.dataIn
    #   
    #   rv$status <- rv$proc$steps.status() 
    # })
    ...
    #       cond <- cond && !(rv$steps.status[x] == global$SKIPPED)
    ...
    ActionOn_Data_Trigger <- function(){
      ...
      if (is.null(unlist(return.values))) { # The entire pipeline has been reseted
        ...
        if (is.null(newValue)){
          ...
          rv$steps.status[ind.processHasChanged:rv$length] <- global$UNDONE
          ...
          validated.steps <- which(rv$steps.status == global$VALIDATED)
          ...
          rv$steps.status[processHasChanged] <- global$VALIDATED
          if (ind.processHasChanged < rv$length)
            rv$steps.status[(1 + ind.processHasChanged):rv$length] <- global$UNDONE
    ...
    ResetChildren = function(range){
      ...
      #browser()
      # lapply(rv$config$steps, function(x){
      #   rv$resetChildren[x] <- 1 + rv$resetChildren[x]
      # })
