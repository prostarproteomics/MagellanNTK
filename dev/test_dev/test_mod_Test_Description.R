...
#--------------------------------------------
source(file.path('example_modules', 'mod_Test_Description.R'), local=TRUE)$value
...
mod_test_process_server <- function(id){
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      rv$dataOut <- mod_nav_process_server(id = 'Test_Description',
    ...
    output$UI <- renderUI({
      mod_nav_process_ui(ns('Test_Description'))
    })
