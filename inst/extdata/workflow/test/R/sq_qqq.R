...
#' 
#' In this example, `sq_qqq_ui()` and `sq_qqq_server()` define
...
wellPanel(
        ...
        # Insert validation button
        uiOutput(ns('sss_btn_validate_ui')),
...
output$sss_btn_validate_ui <- renderUI({
    widget <-  actionButton(ns('sss_btn_validate'),
                   'Perform',
                   class = GlobalSettings$btn_success_color)
      toggleWidget(widget, rv$steps.enabled['sss'] )
      
    })
    # >>> END: Definition of the widgets
    
    
    observeEvent(input$sss_btn_validate, {
      # Do some stuff
      ...
      dataOut$trigger <- Timestamp()
      dataOut$value <- rv$dataIn
      rv$steps.status['sss'] <- global$VALIDATED
    })
  


output$eeee <- renderUI({
...
wellPanel(
        ...
        # Insert validation button
        uiOutput(ns('eeee_btn_validate_ui')),
...
output$eeee_btn_validate_ui <- renderUI({
    widget <-  actionButton(ns('eeee_btn_validate'),
                   'Perform',
                   class = GlobalSettings$btn_success_color)
      toggleWidget(widget, rv$steps.enabled['eeee'] )
      
    })
    # >>> END: Definition of the widgets
    
    
    observeEvent(input$eeee_btn_validate, {
      # Do some stuff
      ...
      dataOut$trigger <- Timestamp()
      dataOut$value <- rv$dataIn
      rv$steps.status['eeee'] <- global$VALIDATED
    })
