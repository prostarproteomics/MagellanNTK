...
Once the process server has been loaded, the UI is opening on the first step of the process ('Description') which explains the content of the process


The server function can be loaded with an object that contains data or a NULL object. In the first case, it means that an object has been sent into the process and the user can work on its data

```{r launchmodule, results='markup', fig.cap="Process overview.", echo=FALSE, out.width='100%', fig.align='center', fig.wide = TRUE}
knitr::include_graphics("./figs/tl_start.png", error = FALSE)
```
...
When the user clicks on the load button on the Description step, then the input object is loaded in the process. The steps become either enabled or disabled w.r.t the rules previously discussed (see xxx).

By default, all steps are `undone`. 

## Navigate through steps

Thanks to the buttons 'Next' and 'Prev', the user can view the different steps without execute them. The current (active) step is marked by an underline below the name of the step. The UI of the current step is shown.
...
## Differences with a basic workflow
