...
**Enabled/disabled tasks**. A disabled task means there is a rule which
...
Given the following example: a process called 'Process 1' which contains
4 steps: *Step1* and *Step2* are third party code and specific to the
process while the steps called *Description* and *Save* are added by
MagellanNTK (to every workflow). The interface of this process is shown
in Fig \@ref(fig:UIprocess).

```{r 'UIprocess', results='markup', fig.cap="User interface for the example process.", echo=FALSE, out.width='100%', fig.align='center', fig.wide = TRUE}
knitr::include_graphics("./figs/UI_process.png", error = FALSE)
```

The interface built by MagellanNTK is composed of three main areas:

-   **Timeline:** represents the sequence of tasks composing the
    process, placed in the order they might be executed.

-   **Commands** (Next, Previous, Reset) allow to interact with the
    whole current step and navigate through the different steps of the
    process.

-   **Current step interface**. The widgets of the UI may be enabled or
    disabled, depending of the state of the process (See section xxx).

### Pipeline workflow

In the case of a pipeline, a second pipeline is needed to navigate
between processes (e.g. in Fig \@ref(fig:UIpipelineannotated), the UI of
a pipeline with the additional timeline)..
...
In this schema, the steps are represented by bullets linked by lines.
The style of both bullets and line gives an information about the state
of the corresponding step. If the bullet of a step is enabled then all
the widgets of this step are enabled (the Perform button also). In the
contrary, a bullet that is disabled means that all the widgets and the
'Perform' button in the UI are disabled
...
interface of what we call 'MagellanNTK core'. It is possible to run this
example by typing the following command in a R console:
...
This will launch a window with the UI of the workflow (Shiny app).
